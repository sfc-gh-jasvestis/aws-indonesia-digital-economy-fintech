...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="AUM (Total)" value={kpiVal('AUM (Total)', 'Rp 12.4T')} status="neutral" />
        <KPICard title="Active Users" value={kpiVal('Active Users', '8.2M')} status="neutral" />
        <KPICard title="NPL Ratio" value={kpiVal('NPL Ratio', '2.1%')} status="warning" />
        <KPICard title="Transactions (MTD)" value={kpiVal('Transactions (MTD)', '47M')} status="neutral" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'Millions' }]}
          title="Transaction Volume (Daily)"
        />
        <Chart
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Approval Rate" value={kpiVal('Approval Rate', '34%')} />
        <KPICard title="Avg Ticket Size" value={kpiVal('Avg Ticket Size', 'Rp 4.2M')} />
        <KPICard title="Collection Rate" value={kpiVal('Collection Rate', '97.8%')} />
      </div>
      <Chart
        data={data?.detail || [{ x: 'Mon', y: 24 }, { x: 'Tue', y: 28 }, { x: 'Wed', y: 22 }, { x: 'Thu', y: 31 }, { x: 'Fri', y: 26 }, { x: 'Sat', y: 19 }, { x: 'Sun', y: 23 }]}
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'NPL %' }]}
        title="NPL Trend by Segment"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          title="OJK Compliance Score"
        />
        <ActionMemo
          persona={{ name: 'Dewi Hartono', role: 'Chief Risk Officer' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Tighten lending criteria for high-NPL segment', 'Submit OJK quarterly report', 'Launch savings product for Gen-Z segment'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which loan segments have rising NPL rates?',
          'Show user acquisition cost by channel',
          'What is the credit scoring model accuracy?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-2', label: 'Regulatory', icon: '⚡', content: domainTab2 },
