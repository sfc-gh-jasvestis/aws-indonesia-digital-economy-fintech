-- ============================================================================
-- 09_AWS_INTEGRATION.SQL — AWS services for Fintech Lending Analytics
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION aws_indonesia_digital_economy_fintech_BEDROCK_EAI
  ALLOWED_NETWORK_RULES = (FINTECH_LENDING.APP.BEDROCK_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (FINTECH_LENDING.APP.AWS_BEDROCK_SECRET)
  ENABLED = TRUE
  COMMENT = 'Bedrock access for Fintech Lending Analytics';
...
CREATE OR REPLACE FUNCTION APP.BEDROCK_GENERATE(prompt VARCHAR)
  ...
  EXTERNAL_ACCESS_INTEGRATIONS = (aws_indonesia_digital_economy_fintech_BEDROCK_EAI)
  SECRETS = ('aws_creds' = FINTECH_LENDING.APP.AWS_BEDROCK_SECRET)
AS $$
...
-- Stream ARN: arn:aws:kinesis:ap-southeast-1:<YOUR_AWS_ACCOUNT_ID>:stream/aws-indonesia-digital-economy-fintech-stream

CREATE OR REPLACE PIPE RAW.REALTIME_PIPE
  AUTO_INGEST = TRUE
  INTEGRATION = 'aws_indonesia_digital_economy_fintech_S3_INT'
  COMMENT = 'Auto-ingest from Kinesis via S3 delivery stream'
AS
COPY INTO RAW.BORROWERS
